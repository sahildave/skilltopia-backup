...
export function estimateReadTimeMinutes(content: string): number {
  const wordCount =
    content.trim() === '' ? 0 : content.trim().split(/\s+/u).length
  ...
  return {
    ...
    async getRawSkillFiles(skillId: string): Promise<Record<string, string>> {
      ...
      for (const row of result.data ?? []) {
        ...
        if (!download.data)
          throw new Error('Supabase repository returned no file data')
...
export function createSupabaseRepositoryFromEnv(): ReturnType<
