...
describe('enrichment pipeline policy', () => {
  it('resolves the configured provider chain and skips missing keys', () => {
    const models = createModelsFromEnv({
      ENRICHMENT_MODEL_CHAIN: 'gemini/gemini-test,groq/llama-test,openai/gpt-test',
      GOOGLE_GENERATIVE_AI_API_KEY: 'google-key',
      GROQ_API_KEY: 'groq-key',
    });

    expect(models).toHaveLength(2);
    expect(models.map((model) => model.modelId)).toEqual(['gemini-test', 'llama-test']);
  });
  ...
  it('force mode re-enriches even when content hash is unchanged', async () => {
    ...
    const repository = {
      getSkillEnrichment: vi.fn().mockResolvedValue({ contentHash: 'sha256:same' }),
  ...
  it('skips existing seed records but re-enriches changed sync records', async () => {
    ...
    const embed = vi.fn();
    const repository = {
      getSkillEnrichment: vi.fn().mockResolvedValue({ contentHash: 'sha256:old' }),
      upsertSkillMetadata,
      putRawSkillFiles,
      upsertSkillEnrichment,
    } as never;
    ...
    await expect(runEnrichmentPipeline({ ...common, mode: 'seed' })).resolves.toMatchObject({
      skipped: 1,
    });
    await expect(runEnrichmentPipeline({ ...common, mode: 'sync' })).resolves.toMatchObject({
      enriched: 1,
    });
