...
describe('web bundle Tauri hard-no', () => {
  ...
  it('lists the agreed markers', () => {
    expect(TAURI_BUNDLE_MARKERS).toEqual([
      '@tauri-apps',
      '__TAURI__',
      '__TAURI_INTERNALS__',
    ])
  ...
  it('finds each marker in text', () => {
    expect(findTauriMarkers('import "@tauri-apps/api"')).toEqual([
      '@tauri-apps',
    ])
    expect(findTauriMarkers('window.__TAURI__ = {}')).toEqual(['__TAURI__'])
    expect(findTauriMarkers('window.__TAURI_INTERNALS__.invoke')).toEqual([
      '__TAURI_INTERNALS__',
    ])
  })
  ...
  it('returns no markers for a clean web chunk', () => {
    expect(
      findTauriMarkers('fetch("/api/skills").then((r) => r.json())')
    ).toEqual([])
  })
  ...
  it('passes a clean bundle and fails a contaminated one', async () => {
    ...
    await expect(assertCleanWebBundle(dirty)).rejects.toThrow(
      /__TAURI_INTERNALS__/
    )
  })
})
