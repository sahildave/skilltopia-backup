...
function readCssValue(block, name) {
  return block?.match(new RegExp(`^\\s*${escapeRegex(name)}\\s*:\\s*([^;]+);`, 'm'))?.[1]?.trim();
}
...
function sync({ checkOnly = false } = {}) {
  ...
  console.log('design: run /sync-design for the semantic prose and component pass');
}
...
function watch() {
  ...
  for (const file of [designPath, ...cssPaths]) fs.watch(file, () => onChange(file));
}
