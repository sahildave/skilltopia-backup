...
- **Updater artifact** — the payload the auto-updater downloads and applies to an
  ...
  Windows the installer, on Linux the AppImage. It is *not* the first-install
  distributable.
- **Installer / DMG** — the distributable a new user downloads for a *first*
...
- **Updater signature** — the minisign signature over an Updater artifact, made
  ...
  public key. An *integrity* control: it proves the artifact is untampered.
- **Developer ID signature** — Apple's code signature plus notarization that lets
  Gatekeeper trust the app on download. A *distribution-trust* control, independent
