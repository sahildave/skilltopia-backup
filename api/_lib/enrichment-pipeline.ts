...
export function createModelsFromEnv(environment = process.env): LanguageModel[] {
  const names = (environment.ENRICHMENT_MODEL_CHAIN ?? DEFAULT_ENRICHMENT_MODEL_CHAIN)
    .split(',')
  ...
  for (const entry of names) {
    ...
    if (provider === 'gemini' || provider === 'google') {
      ...
      const factory =
        providers.get(provider) ??
        (createGoogle({ apiKey }) as unknown as (model: string) => LanguageModel);
      providers.set(provider, factory);
...
export async function runEnrichmentPipeline(
  ...
  for (const [index, skill] of skills.entries()) {
    ...
    try {
      ...
      const markdown = files.find((file) => file.path.toLowerCase() === 'skill.md')?.contents ?? '';
      ...
      log(`${step}: confidence=${confidence}`, confidence === 'llm' ? 'ok' : 'warn');
...
type LocalEnrichmentOptions = Omit<EnrichmentPipelineOptions, 'repository' | 'models' | 'mode'> &
