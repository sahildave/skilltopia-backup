...
describe('detection probe parity', () => {
  it('detects ordinary home-directory providers', () => {
    ...
    expect(evaluateDetection(cursor.detection, contextWithPaths(['/home/user/.cursor']))).toBe(
      true,
    );
    expect(evaluateDetection(cursor.detection, contextWithPaths([]))).toBe(false);
  });
  ...
  it('uses configHome (XDG) for amp detection and skills dir', () => {
    ...
    expect(resolveGlobalSkillsDir(amp.globalSkillsDir, ctx)).toBe('/xdg/agents/skills');
  });
  ...
  it('honors CLAUDE_CONFIG_DIR for claude-code', () => {
    ...
    expect(resolveGlobalSkillsDir(claude.globalSkillsDir, ctx)).toBe('/custom/claude/skills');
  });

  it('detects openclaw via legacy homes and resolves skills dir preferentially', () => {
    const openclaw = requireProvider('openclaw');
    expect(evaluateDetection(openclaw.detection, contextWithPaths(['/home/user/.clawdbot']))).toBe(
      true,
    );

    const legacy = contextWithPaths(['/home/user/.moltbot']);
    expect(resolveOpenClawGlobalSkillsDir(legacy)).toBe('/home/user/.moltbot/skills');
    expect(resolveGlobalSkillsDir(openclaw.globalSkillsDir, legacy)).toBe(
      '/home/user/.moltbot/skills',
    );

    const preferred = contextWithPaths(['/home/user/.openclaw', '/home/user/.clawdbot']);
    expect(resolveOpenClawGlobalSkillsDir(preferred)).toBe('/home/user/.openclaw/skills');
  });
  ...
  it('detects zed via configHome or optional Windows APPDATA', () => {
    ...
    expect(evaluateDetection(zed.detection, contextWithPaths(['/home/user/.config/zed']))).toBe(
      true,
    );
  ...
  it('uses the eve-installed special probe for package.json dependency', () => {
    ...
    expect(evaluateDetection(eve.detection, contextWithPaths(['/proj/agent']))).toBe(true);
  ...
  it('never detects the synthetic universal provider', () => {
    ...
    expect(evaluateDetection(universal.detection, contextWithPaths(['/anything']))).toBe(false);
  });
});
