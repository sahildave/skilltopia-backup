...
describe('parseSkillsLeaderboardQuery', () => {
  ...
  it('rejects invalid view values', () => {
    const result = parseSkillsLeaderboardQuery(new URLSearchParams({ view: 'weekly' }));
  ...
  it('rejects per_page outside 1–500', () => {
    expect(parseSkillsLeaderboardQuery(new URLSearchParams({ per_page: '0' })).ok).toBe(false);
    expect(parseSkillsLeaderboardQuery(new URLSearchParams({ per_page: '501' })).ok).toBe(false);
    expect(parseSkillsLeaderboardQuery(new URLSearchParams({ per_page: '9999' })).ok).toBe(false);
  });

  it('rejects negative page', () => {
    const result = parseSkillsLeaderboardQuery(new URLSearchParams({ page: '-1' }));
    expect(result.ok).toBe(false);
  });
...
describe('parseSkillsSearchQuery', () => {
  ...
  it('rejects unknown keys with 400', () => {
    const result = parseSkillsSearchQuery(new URLSearchParams({ q: 'react', extra: '1' }));
  ...
  it('rejects limit outside 1–200', () => {
    expect(parseSkillsSearchQuery(new URLSearchParams({ q: 'ab', limit: '0' })).ok).toBe(false);
    expect(parseSkillsSearchQuery(new URLSearchParams({ q: 'ab', limit: '201' })).ok).toBe(false);
  });
});

describe('parseSkillDetailQuery', () => {
  it('accepts and cleans a skill id', () => {
    const twoPart = parseSkillDetailQuery(new URLSearchParams({ skill_id: 'owner/my-skill' }));
    ...
    expect(threePart.query.get('skill_id')).toBe('vercel-labs/skills/find-skills');
  });

  it('rejects missing, malformed, and unknown detail params', () => {
    expect(parseSkillDetailQuery(new URLSearchParams()).ok).toBe(false);
    expect(parseSkillDetailQuery(new URLSearchParams({ skill_id: 'my-skill' })).ok).toBe(false);
    expect(
