...
describe('PlatformPort mock', () => {
  ...
  it('exposes a normalized installed scan snapshot', async () => {
    ...
    expect(snapshot.universal.skillCount).toBe(
      MOCK_INSTALLED_SCAN.universal.skillCount
    )
    expect(
      snapshot.skills.some(skill =>
  ...
  it('revealProviderSkillsDir reports missing dirs as false', async () => {
    await expect(
      mockPlatform.revealProviderSkillsDir('claude-code')
    ).resolves.toBe(true)
    await expect(
      mockPlatform.revealProviderSkillsDir('missing-provider')
    ).resolves.toBe(false)
  })
...
describe('PlatformPort web', () => {
  ...
  it('does not reveal provider directories', async () => {
    await expect(
      webPlatform.revealProviderSkillsDir(UNIVERSAL_PROVIDER_ID)
    ).resolves.toBe(false)
  })
