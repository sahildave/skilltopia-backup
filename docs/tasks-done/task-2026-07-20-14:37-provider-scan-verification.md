...
- [x] Add fixture-based parity tests for the generated registry and its
      Universal/non-Universal grouping.
- [x] Add Rust tests for macOS/Windows detection and global path resolution.
- [x] Add shared React tests for provider ordering, counts, warnings, card tag
      merging, alphabetical skills, deduplication, toggle reset, Universal section
      placement, and Rescan refresh behavior.
- [x] Test that selecting a provider only filters the existing in-memory scan
      snapshot and performs no additional filesystem read.
- [x] Test that app-open, Installed Skills activation, and manual Rescan all
      replace one shared snapshot while keeping prior results visible during the
      refresh.
- [x] Test the desktop reveal-path action and missing-directory disabled state.
- [x] Test web and mock adapters, including the absence of Tauri imports in the
      web production graph.
- [x] Run the project quality gate and document any required generated-binding
